...
//func (pd *spiDriver) debugRegisters() {
//	regs := map[string]uint32{
//		"CR":    pd.bus.Bus.CR.Get(),
//		"SR":    pd.bus.Bus.SR.Get(),
//		"IER":   pd.bus.Bus.IER.Get(),
//		"CFGR0": pd.bus.Bus.CFGR0.Get(),
//		"CFGR1": pd.bus.Bus.CFGR1.Get(),
//		"CCR":   pd.bus.Bus.CCR.Get(),
//		"FCR":   pd.bus.Bus.FCR.Get(),
//		"FSR":   pd.bus.Bus.FSR.Get(),
//		"TCR":   pd.bus.Bus.TCR.Get(),
//		"TDR":   pd.bus.Bus.TDR.Get(),
//		"RSR":   pd.bus.Bus.RSR.Get(),
//		"RDR":   pd.bus.Bus.RDR.Get(),
//	}
//	for name, bits := range regs {
//		println(name, bits)
//	}
//}
...
func (pd *spiDriver) setWord(word uint8, first bool, last bool) (ok bool) {
	...
	if last {
		...
		for i := 0; (!canReturn) && (i < tryMax); i++ {
			// println(pd.bus.Bus.SR.Get())
			// println(pd.bus.Bus.FSR.Get())
			// println(pd.bus.Bus.RSR.Get())
